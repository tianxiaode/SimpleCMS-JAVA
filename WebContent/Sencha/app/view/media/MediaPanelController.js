Ext.define('SimpleCMS.view.media.MediaPanelController',{
    ...
    onBeforeUpload: function (cmp, uploader, file) {
        ...
        progress.setValue(0);
        progress.updateText(Ext.String.format(AppLocale.Uploading, file.name, 0));
        tb.show();
    },
    ...
    onUploadProgress: function (cmp1, uploader, filename, size, precent) {
        ...
        progress.updateText(Ext.String.format(AppLocale.Uploading, filename, precent));
    },
    ...
    onUploadError: function (uploader, data) {
        AppToast.toast(Ext.String.format(AppLocale.FileUploadError, data.file.name, data.status && data.status === 200 ? data.file.msg : data.message), this.getView().el);
    },
    ...
    descriptionEditUrl: AppUrl.get('media','update'),
    ...
    mediaDeleteUrl: AppUrl.get('media', 'destroy'),
